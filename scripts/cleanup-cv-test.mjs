...
await sql`delete from cv_documents where user_id in (select id from users where email like 'test.%@example.com')`;
await sql`delete from master_profiles where user_id in (select id from users where email like 'test.%@example.com')`;
